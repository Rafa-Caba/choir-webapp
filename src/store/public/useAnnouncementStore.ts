// src/store/public/useAnnouncementStore.ts
...
interface PublicAnnouncementsState {
    ...
    fetchAnnouncements: (choirCode: string) => Promise<void>;
    reset: () => void;
}
...
export const useAnnouncementStore = create<PublicAnnouncementsState>((set, get) => ({
    ...initialState,
    fetchAnnouncements: async (choirCode) => {
        ...
        try {
            const data = await getPublicAnnouncements(normalizedCode, controller.signal);
